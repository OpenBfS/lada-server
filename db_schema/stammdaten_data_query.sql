...
13	SELECT DISTINCT\n probe.id AS probeId,\n  probe.hauptproben_nr AS hpNr,\n datenbasis.datenbasis AS dBasis,\n mess_stelle.netzbetreiber_id AS netzId,\n CASE\n WHEN\n probe.mst_id = probe.labor_mst_id\n THEN\n probe.mst_id\n ELSE\n probe.mst_id || '-' || probe.labor_mst_id\n END\n AS mstLaborId,\n probe.umw_id AS umwId,\n probe.probeentnahme_beginn AS peBegin,\n probe.probeentnahme_ende AS peEnd,\n probe.ext_id AS externeProbeId,\n status_kombi.id AS statusK,\n umwelt.umwelt_bereich AS umw,\n netz_betreiber.netzbetreiber AS netzbetreiber,\n probe.test,\n betriebsart.name AS messRegime,\n CASE WHEN probe.mst_id = probe.labor_mst_id THEN stamm.mess_stelle.mess_stelle ELSE stamm.mess_stelle.mess_stelle || '-' || labormessstelle.mess_stelle END AS mstLabor,\n staat_uo.staat AS uStaat,\n CASE\n WHEN\n ort_uo.ort_id is null\n OR ort_uo.staat_id = 0\n THEN\n false\n ELSE\n true\n END\n AS sollistIstAusland,\n stamm.sollist_umwgrp.id AS sollistUmwgrpId,\n stamm.sollist_umwgrp.beschreibung AS sollistUmwgrpBeschr,\n stamm.sollist_mmtgrp.bezeichnung AS sollistMmtgrpBez,\n  stamm.sollist_mmtgrp.beschreibung AS sollistMmtgrpBeschr\nFROM\n land.probe\n LEFT JOIN stamm.mess_stelle ON (probe.mst_id = stamm.mess_stelle.id)\n LEFT JOIN stamm.mess_stelle AS labormessstelle ON (probe.labor_mst_id = labormessstelle.id)\n INNER JOIN land.messung ON probe.id = messung.probe_id\n INNER JOIN land.status_protokoll ON messung.STATUS = status_protokoll.id\n LEFT JOIN stamm.status_kombi ON status_protokoll.status_kombi = stamm.status_kombi.id\n LEFT JOIN stamm.status_wert ON stamm.status_wert.id = stamm.status_kombi.wert_id\n LEFT JOIN stamm.status_stufe ON stamm.status_stufe.id = stamm.status_kombi.stufe_id\n LEFT JOIN stamm.datenbasis ON (probe.datenbasis_id = datenbasis.id)\n LEFT JOIN\n land.ortszuordnung AS ortszuordnung_uo\n ON (probe.id = ortszuordnung_uo.probe_id\n AND ortszuordnung_uo.ortszuordnung_typ IN ('U'))\n LEFT JOIN stamm.ort AS ort_uo ON (ortszuordnung_uo.ort_id = ort_uo.id)\n LEFT JOIN stamm.staat AS staat_uo ON (ort_uo.staat_id = staat_uo.id)\n LEFT JOIN stamm.umwelt ON (probe.umw_id = umwelt.id)\n LEFT JOIN stamm.netz_betreiber ON (stamm.mess_stelle.netzbetreiber_id = netz_betreiber.id)\n LEFT JOIN stamm.betriebsart ON (probe.ba_id = stamm.betriebsart.id)\n LEFT JOIN stamm.mess_methode ON (land.messung.mmt_id = stamm.mess_methode.id)\n LEFT JOIN stamm.sollist_umwgrp_zuord ON (probe.umw_id = sollist_umwgrp_zuord.umw_id)\n LEFT JOIN stamm.sollist_umwgrp ON (sollist_umwgrp_zuord.sollist_umwgrp_id = sollist_umwgrp.id AND stamm.sollist_umwgrp.beschreibung <> 'Summe')\n LEFT JOIN stamm.sollist_mmtgrp_zuord ON (messung.mmt_id = sollist_mmtgrp_zuord.mmt_id)\n LEFT JOIN stamm.sollist_mmtgrp ON (sollist_mmtgrp_zuord.sollist_mmtgrp_id = sollist_mmtgrp.id)\n JOIN\n stamm.sollist_soll\n ON (sollist_mmtgrp.id = sollist_soll.sollist_mmtgrp_id\n AND sollist_umwgrp.id = sollist_soll.sollist_umwgrp_id\n AND \n (\n CASE\n WHEN\n ort_uo.ort_id is null\n OR ort_uo.staat_id = 0\n THEN\n false\n ELSE\n true\n END\n)\n = sollist_soll.imp\n AND stamm.netz_betreiber.id = sollist_soll.netzbetreiber_id)\n
...
32	SELECT probenehmer.id, netzbetreiber_id AS netzId, prn_id AS prnId, bearbeiter, bemerkung, betrieb, bezeichnung, kurz_bezeichnung AS kurzBezeichnung, ort, plz, strasse, telefon, tp, typ, letzte_aenderung AS letzteAenderung, netz_betreiber.netzbetreiber AS netzbetreiber FROM stamm.probenehmer LEFT JOIN stamm.netz_betreiber ON (stamm.probenehmer.netzbetreiber_id = netz_betreiber.id)
...
57	SELECT sollist_soll.netzbetreiber_id AS netzId,\n netz_betreiber.netzbetreiber AS netzbetreiber,\n sollist_mmtgrp.bezeichnung AS sollistMmtgrpBez,\n sollist_mmtgrp.beschreibung AS sollistMmtgrpBeschr,\n sollist_soll.sollist_umwgrp_id AS sollistUmwgrpId,\n sollist_umwgrp.beschreibung  AS sollistUmwgrpBeschr,\n sollist_soll.imp AS sollistSollImp,\n sollist_soll.soll AS sollistSoll\n FROM stamm.sollist_soll\n LEFT JOIN stamm.netz_betreiber ON (sollist_soll.netzbetreiber_id = netz_betreiber.id)\n LEFT JOIN stamm.sollist_mmtgrp ON (sollist_soll.sollist_mmtgrp_id = sollist_mmtgrp.id)\n LEFT JOIN stamm.sollist_umwgrp ON (sollist_soll.sollist_umwgrp_id = sollist_umwgrp.id)
58	SELECT mess_methode.messmethode AS mmt,\n sollist_mmtgrp.bezeichnung AS sollistMmtgrpBez,\n sollist_mmtgrp.beschreibung AS sollistMmtgrpBeschr\n FROM stamm.sollist_mmtgrp_zuord\n LEFT JOIN stamm.sollist_mmtgrp ON (sollist_mmtgrp_zuord.sollist_mmtgrp_id = sollist_mmtgrp.id)\n LEFT JOIN stamm.mess_methode ON (sollist_mmtgrp_zuord.mmt_id = mess_methode.id)
59	SELECT umw_id AS umwId,\n umwelt.umwelt_bereich AS umw,\n sollist_umwgrp_zuord.sollist_umwgrp_id AS sollistUmwgrpId,\n sollist_umwgrp.beschreibung sollistUmwgrpBeschr\n FROM stamm.sollist_umwgrp_zuord\n LEFT JOIN stamm.sollist_umwgrp ON (sollist_umwgrp_zuord.sollist_umwgrp_id = sollist_umwgrp.id)\n LEFT JOIN stamm.umwelt ON (sollist_umwgrp_zuord.umw_id = umwelt.id)
...
23	Soll/Ist Übersicht	1	13	Vorlage für Soll/Ist Übersicht
...
28	StammBund Soll/Ist Soll	1	57	Abfrage Soll/Ist Soll
29	StammBund Soll/Ist Messmethoden	1	58	Abfrage Soll/Ist Messmethoden
30	StammBund Soll/Ist Umwelt	1	59	Abfrage Soll/Ist Umwelt
...
623	1	1301	23	\N	\N	\N	f	f	\N	95
624	1	1302	23	\N	\N	\N	f	f	\N	95
625	1	1303	23	\N	\N	2,10	t	f	\N	80
626	1	1304	23	\N	\N	\N	f	f	\N	50
627	1	1305	23	\N	\N	\N	f	f	\N	95
628	1	1306	23	\N	\N	\N	f	f	\N	55
629	1	1307	23	\N	\N	\N	t	f	\N	125
630	1	1308	23	\N	\N	\N	f	f	\N	125
631	1	1309	23	asc	4	\N	f	t	4	145
632	1	1310	23	\N	\N	10,11	t	f	\N	145
633	1	1311	23	\N	\N	\N	f	f	\N	150
634	1	1312	23	\N	\N	\N	t	f	\N	120
635	1	1313	23	\N	\N	false	t	f	\N	40
636	1	1314	23	\N	\N	1	t	f	\N	150
637	1	1315	23	\N	\N	\N	f	f	\N	165
638	1	1316	23	\N	\N	\N	t	f	\N	80
639	1	1317	23	asc	1	\N	f	t	0	100
640	1	1318	23	asc	2	\N	f	t	1	100
641	1	1319	23	\N	\N	\N	t	t	2	280
642	1	1320	23	asc	3	\N	f	t	3	180
...
828	1	1321	23	\N	\N	\N	t	f	\N	180
829	1	5701	28	asc	1	\N	f	t	1	60
830	1	5702	28	\N	\N	\N	t	t	2	180
831	1	5703	28	asc	4	\N	f	t	5	150
832	1	5704	28	\N	\N	\N	t	t	6	180
833	1	5705	28	asc	3	\N	f	t	3	150
834	1	5706	28	\N	\N	\N	t	t	4	200
835	1	5707	28	asc	2	\N	f	t	0	120
836	1	5708	28	\N	\N	\N	f	t	7	80
837	1	5801	29	\N	\N	\N	t	t	0	220
838	1	5802	29	asc	\N	\N	f	t	1	200
839	1	5803	29	\N	\N	\N	t	t	2	180
840	1	5901	30	\N	\N	\N	f	t	0	80
841	1	5902	30	\N	\N	\N	t	t	1	280
842	1	5903	30	asc	\N	\N	f	t	2	150
843	1	5904	30	\N	\N	\N	t	t	3	250
